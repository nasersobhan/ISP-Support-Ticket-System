...
//    }
  ...
  $ret= cat_search_inui($getVar,is_get('type'));
...
//        $wherepart ='';}
//        
//$sql = "SELECT cat_id, cat_name,cat_type FROM {$pref}categories_oy WHERE cat_name like '%" . $getVar ."%' {$wherepart} AND (cat_status=1 OR cat_uid={$uid}) ORDER BY cat_name ASC";
...
//                $row_array['text'] =($row['cat_name'] .' ('.get_cate_name($row['cat_type']).')'); //utf8_encode(($row['cat_name']));
