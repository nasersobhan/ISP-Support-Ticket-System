...
$fragsxx = explode(".", $src);
$ext = strtolower( $fragsxx[ count( $fragsxx ) - 1 ] );

$local_file = realpath('./tmp/').'/'.basename($src).rand(0,99999).'-x.'.$ext ;
$copy = copy($src, $local_file );
$src =$local_file;
...
$zoom_crop = $zoom_crop	= preg_replace( "/[^0-9]/", "", $_REQUEST[ 'zc' ] );
...
// get path to image on file system  $_SERVER['DOCUMENT_ROOT'] . '/' .
