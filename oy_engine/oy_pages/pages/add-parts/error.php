...
global $add_form,$excp, $dbase;
  ...
  if(is_get('oid'))
   ...
   $edit = FALSE;
    ...
    $fields = 'S:'.  g_lbl('subtitle').'::S,name,desc,category,cause,language,level,link,avatar';
    ...
    $excp['desc'] = array('label' => '','name'=>'content[desc]', 'type' => 'area', 'att' => ' rows="15" ', 'class' => 'form-control editor');
  ...
  //  $excp['author'] = array('label' => g_lbl('author'),'options'=>array('',''),'name'=>'content[author]','att'=>'data-type="author"', 'type' => 'select', 'class' => 'form-control cate2au');
   $excp['link'] = array('label' => g_lbl('link'),'options'=>array('',''),'name'=>'content[link]', 'type' => 'text', 'class' => 'form-control');
   
    $excp['cause'] = array('label' => g_lbl('cause'),'options'=>array('',''),'name'=>'content[cause]', 'type' => 'text', 'class' => 'form-control');
   $excp['avatar'] = array('label' => g_lbl('thumb').':','type'=>'file', 'class' => 'form-control ');
     $cates = cat_2arr_l('error_type','fa_AF'); // $dbase->tbl2array(TBL_PIX . 'categories_oy', 'cat_id', 'cat_name', " WHERE cat_lang='{$lang}' AND cat_type = 'deals' AND cat_parent='{$parent}' AND ((cat_uid=0) OR (cat_uid=" . user_uid() . "))");
    $excp['category'] = array('label' => g_lbl('category'),'options'=>$cates,'att'=>'data-type="author"', 'type' => 'select', 'class' => 'form-control');
      $lvl = array(0=>'low',1=>'Med',2=>'High');
    $excp['level'] = array('label' => g_lbl('level'),'options'=>$lvl,'name'=>'content[level]', 'type' => 'select', 'class' => 'form-control');
      $cates = cat_2arr_l('language','fa_AF');
    $excp['language'] = array('label' => g_lbl('language'),'options'=>$cates,'name'=>'content[language]', 'type' => 'select', 'class' => 'form-control');
