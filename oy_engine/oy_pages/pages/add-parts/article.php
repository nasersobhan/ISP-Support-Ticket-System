...
global $add_form,$excp, $dbase;
  ...
  if(is_get('oid'))
   ...
   $edit = FALSE;
    ...
    $fields = 'S:'.  g_lbl('subtitle').'::S,name,desc,category,author,tags,src,avatar';
   ...
   $excp['src'] = array('label' => g_lbl('sourcelink'),'options'=>array('',''),'name'=>'content[src]', 'type' => 'text', 'class' => 'form-control');
   $excp['avatar'] = array('label' => g_lbl('thumb').':','type'=>'file', 'class' => 'form-control ');
     $cates = cat_2arr_l('article_cat','fa_AF'); // $dbase->tbl2array(TBL_PIX . 'categories_oy', 'cat_id', 'cat_name', " WHERE cat_lang='{$lang}' AND cat_type = 'deals' AND cat_parent='{$parent}' AND ((cat_uid=0) OR (cat_uid=" . user_uid() . "))");
